...
class net_epcl(pl.LightningModule):
    def __init__(self, args=None, weights_train=None, weights_test=None):
        super().__init__()
    ...
    def forward(self, data_dict):
        pred_dict = {}
        loss_dict = {}
        # print(type(data_dict))  # 检查是否是字典
        # print(data_dict.keys() if isinstance(data_dict, dict) else "不是字典")
        with torch.no_grad():
        ...
        weight = self.weights_train if is_train else self.weights_test
    ...
    def training_step(self, data_dict, batch_idx):
        ...
        if (self.global_rank == 0) and (self.global_step % 1000 == 0):
            ...
            if self.logger is not None:
                self.log("nvox_train", nvox, prog_bar=True, on_step=True, on_epoch=True)
                self.log("current_epoch_train", self.current_epoch, prog_bar=True, on_step=False, on_epoch=True)
                self.log("global_step_train", self.global_step, prog_bar=False, on_step=True, on_epoch=False)
                self.log("lr_train", self.scheduler.get_last_lr()[0], prog_bar=False, on_step=True, on_epoch=False)
    ...
    def on_validation_epoch_end(self):
        with torch.no_grad():
            ...
            self.log("mIoU_val", mIoU, sync_dist=True)
            ...
            if self.global_rank == 0:
                if self.logger is not None:
                    ...
                    self.log('val_mAcc', mAcc, sync_dist=True)

                    self.log('val_allAcc', allAcc, sync_dist=True)

                    self.log('epoch', self.current_epoch, sync_dist=True)

                    self.log('lr', self.scheduler.get_last_lr()[0], sync_dist=True)  # 假设 get_last_lr() 返回一个列表

                    # 如果需要记录当前 epoch 和 global step

                    self.log('current_epoch', self.current_epoch, sync_dist=True)

                    self.log('global_step', self.global_step, sync_dist=True)

                    self.log('lr_val', self.scheduler.get_last_lr()[0], sync_dist=True)
                if self.text_logger is not None:
                    str_to_print = (
    ...
    def on_test_epoch_end(self):
        with torch.no_grad():
            ...
            if self.global_rank == 0:
                if self.logger is not None:
                    self.log("mIoU_test_per_scene", mIoU, prog_bar=True, sync_dist=True)
                    self.log("mAcc_test_per_scene", mAcc, prog_bar=True, sync_dist=True)
                    self.log("allAcc_test_per_scene", allAcc, prog_bar=True, sync_dist=True)
